...
{ 
    int i, j; 
    float key2;
    for (i = iter; i < NUM_BARRAS; i++) {
        Barra b1 = barras.get(i);
        key2 = b1.valor; 
        j = i - 1; 
        Barra b2 = barras.get(j);
        // Move elements of arr[0..i-1],  
        // that are greater than key, to one 
        // position ahead of their 
        // current position
        while (j >= 0 && b2.valor > key2) {
            Barra b3 = barras.get(j+1);
            b3.valor = b2.valor;
            j = j - 1;
            if ( j >= 0) b2 = barras.get(j);
        }
            
        Barra b4 = barras.get(j+1);
        b4.valor = key2;
        return ;
    }
    fill(#00FF00);
...
void draw() {
  ...
  // ADDED DELAY TO SELECTION 
  //if (int(random(0,5)) == 0) selectionSort();
  if (int(random(0,5)) == 0) {insertionSort(); iter++;}
